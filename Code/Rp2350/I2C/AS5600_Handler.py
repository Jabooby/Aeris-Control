...
class AS5600:
    ...
    async def _read_register(self, reg: int, length: int = 1):
        """
        ...
        """
        return await asyncio.to_thread(self.i2c.readfrom_mem, self.address, reg, length)
    
    async def _write_register(self, reg: int, data: bytes):
        """
        Helper function to write data to a register
        :param reg: Register address to write to
        :param data: Data to write (bytes)
        """
        await asyncio.to_thread(self.i2c.writeto_mem, self.address, reg, data)
    ...
    async def get_angle(self):
        ...
        angle_high = await self._read_register(AS5600_REG_ANGLE_HIGH)
        angle_low = await self._read_register(AS5600_REG_ANGLE_LOW)
    ...
    async def get_magnitude(self):
        """
        Get the magnetic field magnitude.
        :return: magnitude (int)
        """
        # Read high and low byte of magnitude
        magnitude_high = await self._read_register(AS5600_REG_MAGNITUDE_HIGH)
        magnitude_low = await self._read_register(AS5600_REG_MAGNITUDE_LOW)
    ...
    async def is_ready(self):
        ...
        """
        status = await self._read_register(AS5600_REG_STATUS)
...
if __name__ == "__main__":        # Setup I2C
    async def main():
        # Initialize I2C
        i2c = I2C(0, sda=Pin(0), scl=Pin(1), freq=100_000)  # Adjust pin numbers as needed
        ...
        # Main loop
        while True:
            if await sensor.is_ready():
                angle = await sensor.get_angle()
                print(f"Angle: {angle:.2f}°")
                
                magnitude = await sensor.get_magnitude()
